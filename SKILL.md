...
**Антиботы смотрят не на fingerprint — они смотрят на поведение.** Реальный человек и бот отличаются не IP или User-Agent, а *ритмом*: паузы, траектория мыши, порядок заполнения, ошибки. Если ты ведёшь себя как бот — тебя заблокируют, даже с идеальным fingerprint.

Ниже — выжимка из антидетект-индустрии и наших профилей. Следуй этому на каждой сессии.

### Общий принцип

```
Бот:         тип-тип-клик-скролл-клик — всё за 0.5с → 🚫 BLOCKED
Человек:     open → пауза 1-3с → скролл чуть-чуть → пауза → тип... → пауза → клик → ✅ PASS
```

Каждое действие должно иметь:
1. **Случайную паузу перед ним** (человек тянется к мышке, смотрит в экран, думает)
2. **Естественную траекторию** (мышь не телепортируется)
3. **Логичную последовательность** (человек заполняет поля по порядку, сверху вниз)

### Таблица: бот vs человек

| Действие | ❌ Бот (блок) | ✅ Человек (пройдёт) |
|----------|--------------|-------------------|
| **Открытие страницы** | Сразу начинает кликать | Пауза **800-2500ms** — «читает», «осматривается» |
| **Клик по кнопке** | Мышь телепортируется мгновенно | Мышь идёт по кривой, **200-600ms** «наведения» перед кликом |
| **Заполнение формы** | Все поля за 1-2 секунды | Поле за полем, **300-900ms** между полями, скролл между секциями |
| **Печать текста** | Текст появляется мгновенно | `ceki type --natural` — клавиши с паузами, **80-110 WPM**, иногда ошибается |
| **Скролл** | `scrollTo(0, documentHeight)` — прыжок в конец | **2-3 прохода**, пауза **400-1500ms** между проходами. Прочитал кусок → скролл дальше |
| **Чекбоксы / радио** | Кликает без паузы, идеально в центр | Легкий оффсет, **300-700ms** на прицеливание |
| **Ошибка при вводе** | Никогда не ошибается | **~3% keystrokes** — опечатка → backspace → исправление |
| **Селект / дропдаун** | Выбирает первую опцию мгновенно | Скроллит список, выбирает не спеша, **500-1500ms** |
| **Мультизадачность** | Клик-скролл-тип параллельно | **Одно действие за раз.** Человек не кликает во время печати |
| **Паузы между действиями** | Всегда одинаковые | **Случайные**, отличаются в 2-3 раза (норма: 300-3000ms) |

### Профили поведения

У нас есть готовые профили в `profiles/*.json`. Каждый профиль —预设 (пресет) поведения под конкретный типаж. CLI автоматом выбирает профиль через `CEKI_HUMAN_PROFILE`. **Не используй `--no-human` без причины.**

| Параметр профиля | Что контролирует | Urban Professional | College Student |
|---|---|---|---|
| `typing.wpm` | Скорость печати | 80-110 (быстро) | 50-80 (медленнее) |
| `typing.backspace_freq` | Частота опечаток | 3% | 7% |
| `typing.think_ms` | Пауза «задумался» | 300-1000ms | 500-2000ms |
| `scroll.speed_px_per_ms` | Скорость скролла | 25-60 px/ms | 15-40 px/ms |
| `scroll.pause_between_passes` | Пауза между проходами | 400-1500ms | 600-2500ms |
| `mouse.curvature` | Кривизна траектории | 0.15 (уверенная) | 0.35 (неточнее) |
| `mouse.jitter` | Дрожание мыши | 0.3 | 0.5 |
| `navigation.pre_click_pause` | Заминка перед кликом | 200-600ms | 300-900ms |

### Скрипт-контроль: что делать на каждой сессии

```
1. PAGE LOAD
   → Ничего не делать 1-3 секунды. Страница грузится, человек смотрит.
   
2. FIRST SCROLL
   → Скролл вниз на 20-40% экрана. Пауза 500-1500ms.
   → Ещё скролл, если контент длинный.
   
3. FIRST CLICK
   → Навести мышь (~300ms кривая). Клик. Ждать 500-1000ms.
   
4. FORM FILL
   → Поле за полем. Между полями пауза 300-900ms.
   → После 3-4 полей — скролл вниз, пауза «проверить».
   
5. SUBMIT
   → Перед сабмитом — пауза 500-1500ms («проверил форму»).
   → Клик. Ждать ответа 2-5 секунд.
```

### ⚠️ Red flags (что тригерит антиботы)

| Триггер | Почему |
|---------|--------|
| **Нулевые паузы между действиями** | Человек физически не может кликать быстрее 100ms |
| **Параллельные действия** | Человек не скроллит и печатает одновременно |
| **Телепорт мыши** | CDP `Input.dispatchMouseEvent` без промежуточных точек |
| **Заполнение снизу вверх / не по порядку** | Человек идёт по порядку в DOM |
| **Идеальный ввод без ошибок** | У всех бывают опечатки |
| **Одинаковое время между всеми действиями** | Слишком регулярно, не похоже на человека |
| **Мгновенный сабмит после загрузки страницы** | Человек хотя бы скроллит перед отправкой |
| **Клик в пиксель 0,0 или края экрана** | Человек так не кликает |

### Итог

```diff
+ Если сомневаешься — добавь паузу. Лучше медленно и пройдёшь, чем быстро и заблокируют.
+ Если видишь CAPTCHA — это не ошибка инструмента, это поведение. Добавь больше пауз, скроллов, случайности.
+ После бана — смени профиль (`CEKI_HUMAN_PROFILE`) и уменьши скорость.
```
